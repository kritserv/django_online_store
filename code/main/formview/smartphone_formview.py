...
class SmartphoneFormView(FormView):
    ...
    def form_valid(self, form):
        ...
        (
            ...
            all_color_query,
            all_processor_query,
            all_processor_brand_query,
            all_ram_query,
            all_storage_query,
            all_os_query,
            all_front_camera_query,
            all_back_camera_query,
            all_sim_query,
        ) = ([], [], [], [], [], [], [], [], [], [], [], [], [])
        ...
        for query in form.cleaned_data["product_available"]:
            if query:
                ...
                for x in filtered_smartphone.filter(is_in_stock=query).values_list(
                    "id"
                ):
                    all_is_in_stock_query.append(x[0])
        if form.cleaned_data["product_available"]:
            filtered_smartphone = filtered_smartphone.filter(
                id__in=all_is_in_stock_query
            )
        query = None
        ...
        for query in form.cleaned_data["flagship"]:
            if query:
                at_least_1_query = True
                for x in filtered_smartphone.filter(is_flagship=query).values_list(
                    "id"
                ):
        ...
        for query in form.cleaned_data["processor_brand"]:
            if query:
                at_least_1_query = True
                for x in filtered_smartphone.filter(processor_brand=query).values_list(
                    "id"
                ):
                    all_processor_brand_query.append(x[0])
        if form.cleaned_data["processor_brand"]:
            filtered_smartphone = filtered_smartphone.filter(
                id__in=all_processor_brand_query
            )
        query = None
        ...
        for query in form.cleaned_data["front_camera"]:
            if query:
                at_least_1_query = True
                for x in filtered_smartphone.filter(front_camera=query).values_list(
                    "id"
                ):
                    all_front_camera_query.append(x[0])
        if form.cleaned_data["front_camera"]:
            filtered_smartphone = filtered_smartphone.filter(
                id__in=all_front_camera_query
            )

        for query in form.cleaned_data["back_camera"]:
            if query:
                at_least_1_query = True
                for x in filtered_smartphone.filter(back_camera=query).values_list(
                    "id"
                ):
                    all_back_camera_query.append(x[0])
        if form.cleaned_data["back_camera"]:
            filtered_smartphone = filtered_smartphone.filter(
                id__in=all_back_camera_query
            )
        ...
        smartphone_link = [
            "/smartphone/product/" + str(x[0])
            for x in filtered_smartphone.values_list("id")
        ]
        smartphone_onsale = [
            x[0] for x in filtered_smartphone.values_list("is_on_sale")
        ]
        smartphone_og_price = [
            x[0] for x in filtered_smartphone.values_list("og_price")
        ]
        smartphone_price = [x[0] for x in filtered_smartphone.values_list("price")]
        smartphone_is_in_stock = [
            x[0] for x in filtered_smartphone.values_list("is_in_stock")
        ]
        smartphone_in_stocks = [
            x[0] for x in filtered_smartphone.values_list("in_stocks")
        ]
        smartphone_is_recommend = [
            x[0] for x in filtered_smartphone.values_list("is_recommend")
        ]
        ...
        for i in range(len(smartphone_title)):
            ...
            smartphone_data.append(
                {
                    "title": smartphone_title[i],
                    "onsale": smartphone_onsale[i],
                    "ogprice": smartphone_og_price[i],
                    "price": smartphone_price[i],
                    "im": smartphone_img[i],
                    "instock": smartphone_is_in_stock[i],
                    "available": smartphone_in_stocks[i],
                    "link": smartphone_link[i],
                    "recommend": smartphone_is_recommend[i],
                    "star": fullstar,
                    "star_num": " (" + smartphone_star[i] + ")",
                }
            )
