...
class HeadphoneFormView(FormView):
    ...
    def form_valid(self, form):
        ...
        (
            ...
            all_is_bluetooth_query,
            all_is_waterproof_query,
            all_has_noise_cancelling_query,
            all_has_microphone_query,
            all_color_query,
            all_headphone_type_query,
            all_port_query,
            all_frequency_response_query,
        ) = ([], [], [], [], [], [], [], [], [], [])
        ...
        for query in form.cleaned_data["is_bluetooth"]:
            if query:
                at_least_1_query = True
                for x in filtered_headphone.filter(is_bluetooth=query).values_list(
                    "id"
                ):
                    all_is_bluetooth_query.append(x[0])
        if form.cleaned_data["is_bluetooth"]:
            filtered_headphone = filtered_headphone.filter(
                id__in=all_is_bluetooth_query
            )
        query = None
        ...
        for query in form.cleaned_data["is_waterproof"]:
            if query:
                at_least_1_query = True
                for x in filtered_headphone.filter(is_waterproof=query).values_list(
                    "id"
                ):
                    all_is_waterproof_query.append(x[0])
        if form.cleaned_data["is_waterproof"]:
            filtered_headphone = filtered_headphone.filter(
                id__in=all_is_waterproof_query
            )
        query = None
        ...
        for query in form.cleaned_data["has_noise_cancelling"]:
            if query:
                at_least_1_query = True
                for x in filtered_headphone.filter(
                    has_noise_cancelling=query
                ).values_list("id"):
                    all_has_noise_cancelling_query.append(x[0])
        if form.cleaned_data["has_noise_cancelling"]:
            filtered_headphone = filtered_headphone.filter(
                id__in=all_has_noise_cancelling_query
            )
        query = None
        ...
        for query in form.cleaned_data["has_microphone"]:
            if query:
                at_least_1_query = True
                for x in filtered_headphone.filter(has_microphone=query).values_list(
                    "id"
                ):
                    all_has_microphone_query.append(x[0])
        if form.cleaned_data["has_microphone"]:
            filtered_headphone = filtered_headphone.filter(
                id__in=all_has_microphone_query
            )
        query = None
        ...
        for query in form.cleaned_data["headphone_type"]:
            if query:
                at_least_1_query = True
                for x in filtered_headphone.filter(headphone_type=query).values_list(
                    "id"
                ):
                    all_headphone_type_query.append(x[0])
        if form.cleaned_data["headphone_type"]:
            filtered_headphone = filtered_headphone.filter(
                id__in=all_headphone_type_query
            )
        query = None
        ...
        headphone_link = [
            "/headphone/product/" + str(x[0])
            for x in filtered_headphone.values_list("id")
        ]
        ...
        headphone_in_stocks = [
            x[0] for x in filtered_headphone.values_list("in_stocks")
        ]
