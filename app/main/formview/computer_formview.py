...
class ComputerFormView(FormView):
    ...
    def form_valid(self, form):
        ...
        for query in form.cleaned_data["gaming"]:
            if query:
                at_least_1_query = True
                for x in filtered_computer.filter(is_gamingtype=query).values_list(
                    "id"
                ):
        ...
        computer_link = [
            "/computer/product/" + str(x[0])
            for x in filtered_computer.values_list("id")
        ]
        ...
        computer_is_in_stock = [
            x[0] for x in filtered_computer.values_list("is_in_stock")
        ]
        computer_in_stocks = [x[0] for x in filtered_computer.values_list("in_stocks")]
        computer_is_recommend = [
            x[0] for x in filtered_computer.values_list("is_recommend")
        ]
        ...
        for i in range(len(computer_title)):
            ...
            computer_data.append(
                {
                    "title": computer_title[i],
                    "onsale": computer_onsale[i],
                    "ogprice": computer_og_price[i],
                    "price": computer_price[i],
                    "im": computer_img[i],
                    "instock": computer_is_in_stock[i],
                    "available": computer_in_stocks[i],
                    "link": computer_link[i],
                    "recommend": computer_is_recommend[i],
                    "star": fullstar,
                    "star_num": " (" + computer_star[i] + ")",
                }
            )
